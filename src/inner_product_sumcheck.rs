...
pub struct ProductSumcheck<F: SumcheckRing> {
...
fn dot<F: SumcheckRing>(a: &[F], b: &[F]) -> F {
...
pub fn compute_sumcheck_polynomial<F: SumcheckRing>(a: &[F], b: &[F]) -> (F, F) {
    fn recurse<F: SumcheckRing>(a0: &[F], a1: &[F], b0: &[F], b1: &[F]) -> (F, F) {
...
pub fn fold_and_compute_polynomial<F: SumcheckRing>(
    a: &mut Vec<F>,
    b: &mut Vec<F>,
    weight: F,
) -> (F, F) {
