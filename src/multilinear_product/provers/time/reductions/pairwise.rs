...
pub fn product_evaluate<F: Field>(src: &[Vec<F>], inverse_four: F) -> (F, F, F) {
    let len = src[0].len();
    ...
    let (acc00, acc01, acc10, acc11): (F, F, F, F) = cfg_into_iter!(0..len)
        .step_by(2)
        .fold(
...
pub fn product_evaluate_from_stream<F: Field, S: Stream<F>>(
    src: &[S],
    inverse_four: F,
) -> (F, F, F) {
    ...
    let (acc00, acc01, acc10, acc11): (F, F, F, F) = cfg_into_iter!(0..len)
        ...
        .fold(
            ...
            |(mut j00, mut j01, mut j10, mut j11), i| {
                ...
                let p1 = p_evals.evaluation(i + 1);
                let q0 = q_evals.evaluation(i);
                let q1 = q_evals.evaluation(i + 1);
                j00 += p0 * q0; // (0,0)
                j11 += p1 * q1; // (1,1)
                j01 += p0 * q1; // (0,1)
                j10 += p1 * q0; // (1,0)
