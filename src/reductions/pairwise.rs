...
pub fn evaluate<F: SumcheckRing>(src: &[F]) -> (F, F) {
...
pub fn reduce_evaluations<F: SumcheckRing>(src: &mut Vec<F>, verifier_message: F) {
...
pub fn pairwise_product_evaluate<F: SumcheckRing>(src: &[Vec<F>]) -> (F, F) {
...
pub fn cross_field_reduce<BF: SumcheckRing, EF: SumcheckRing + From<BF>>(
    src: &[BF],
    challenge: EF,
) -> Vec<EF> {
    cfg_chunks!(src, 2)
