...
mod tests {
    ...
    #[test]
    fn to_evaluations_from_evaluations_sanity_small_fp() {
        // we should get back the same polynomial
        let p1: SparsePolynomial<SmallF19, SparseTerm> = four_variable_polynomial::<SmallF19>();
        let p1_evaluations: Vec<SmallF19> = p1.to_evaluations();
        assert_eq!(
            p1,
        ...
        // we should get back the same evaluations
        let num_variables: usize = 16;
        let s: BenchStream<SmallF19> = BenchStream::new(num_variables);
        let hypercube_len: usize = 2usize.pow(num_variables as u32);
        let mut p2_evaluations: Vec<SmallF19> = Vec::with_capacity(hypercube_len);
        for i in 0..hypercube_len {
            p2_evaluations.push(s.evaluation(i));
        }
        let p2: SparsePolynomial< SmallF19, SparseTerm> =
            <SparsePolynomial< SmallF19, SparseTerm> as Polynomial< SmallF19>>::from_hypercube_evaluations(
                p2_evaluations.clone(),
            );
        assert_eq!(p2_evaluations, p2.to_evaluations());
